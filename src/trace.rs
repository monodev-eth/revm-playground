...
async fn trace_state_diff(
    ...
    info!(
        "Tx #{} recieved. Checking if it touches: {}",
...
pub async fn mempool_watching(target_address: String) -> Result<()> {
    //Setup: Create te WS provider and wrap it in Arc
    ...
    //Step #2 Steream data sync
    let (event_sender, _): (Sender<Event>, _) = broadcast::channel(512);

    let mut set = JoinSet::new();

    //Stream Block Heaaders
    {
    ...
    //Catch events
    {
        let mut event_reciever = event_sender.subscribe();
